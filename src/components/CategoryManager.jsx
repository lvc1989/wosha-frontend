    ...
    <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-6 max-w-lg mt-6">
      ...
      {categories.map((c) => (
        <div key={c.id} className="flex items-center justify-between py-2" style={{ borderTop: `1px solid ${C.border}` }}>
          ...
          <button onClick={() => remove(c.id)} className="text-xs font-semibold" style={{ color: C.amberDeep }}>Remove</button>
        </div>
      ))}
      ...
      <form onSubmit={add} className="flex flex-wrap gap-2 mt-3">
        <input placeholder="New category name" value={newName} onChange={(e) => setNewName(e.target.value)} style={{ borderColor: C.border, minWidth: 160 }} className="flex-1 border rounded-lg px-3 py-2 text-sm" />
