...
import { PrintFooter } from "../PrintHeaderFooter.jsx";
import { ListRow, StatusPill, Button, Modal, FieldLabel, LoadingState } from "../ui.jsx";

const money = (n) => "TZS " + Number(n || 0).toLocaleString();
const PAYMENT_METHODS = ["Cash", "Control Number", "Mobile Money", "Bank Transfer"];

// Original Invoicing.jsx, exactly — only the outer PageHeader was removed. The
// printed-receipt structure below (.wosha-printable) is copied character for
// character and must never be restructured — the print stylesheet depends on
// this exact DOM shape.
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div style={{ color: C.textSoft }} className="text-xs">Showing {invoices.length} of {total}{!dateFrom && !dateTo ? " · most recent" : ""}</div>
      </div>
          ...
          <div style={{ background: "#fff" }} className="wosha-printable w-full max-w-md rounded-xl p-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-6 pb-6" style={{ borderBottom: "2px solid " + C.ink }}>
              ...
              <div className="text-right">
                ...
                <span style={{ background: receipt.status === "Paid" ? "#E6F1FB" : "#FDE8E7", color: receipt.status === "Paid" ? "#185FA5" : C.danger }} className="inline-block text-xs font-semibold px-2.5 py-1 rounded-full mt-2">{receipt.status}</span>
              </div>
            </div>
            ...
            <table className="w-full text-sm mb-4" style={{ borderCollapse: "collapse", tableLayout: "fixed" }}>
              <colgroup>
                <col style={{ width: "55%" }} />
                <col style={{ width: "15%" }} />
                <col style={{ width: "30%" }} />
              ...
              <tbody>
                {(receipt.items || []).map((it, idx) => (
                  <tr key={idx} style={{ borderBottom: "1px solid #F1F2F4" }}>
                    <td className="py-2" style={{ color: C.ink, wordBreak: "break-word" }}>{it.name}</td>
