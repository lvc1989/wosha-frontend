...
export default function ExpensesTab() {
  const { user } = useUser();
  const canRemove = user?.role === "owner" || user?.role === "manager";
  ...
  const remove = async (id) => { if (confirm("Remove this expense? This can't be undone.")) { await api.removeExpense(id); load(); } };
                ...
                trailing={
                  e.status === "Pending Approval" ? (
                    <div className="flex items-center gap-2 flex-wrap justify-end">
                      ...
                      {canRemove && <button onClick={() => remove(e.id)} style={{ color: C.textSoft }}><X size={14} /></button>}
                    </div>
                  ) : (
