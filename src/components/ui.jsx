...
export function QuickActionGrid({ items }) {
  ...
  return (
    <div className={"grid gap-2.5 " + (pref.cols === 2 ? "grid-cols-2" : "grid-cols-3")}>
      {items.map((it) => {
        ...
        return (
          ...
          >
            <div style={{ background: t.tint }} className={`${preset.box} ${preset.radius} flex items-center justify-center`}>
              <it.icon size={preset.icon} color={t.deep} strokeWidth={1.9} />
            </div>
...
// A closed-by-default expandable section — used to group related settings
// (like several color pickers) so they don't all compete for attention at
// once. Click the header to open; click it again, or open a sibling, to close.
export function CollapsibleSection({ title, subtitle, defaultOpen = false, children, right }) {
  const [open, setOpen] = React.useState(defaultOpen);
  return (
    <div className="border-b last:border-b-0" style={{ borderColor: C.border }}>
      <button type="button" onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between gap-3 py-4 text-left">
        <div>
          <div style={{ color: C.ink }} className="text-sm font-semibold">{title}</div>
          {subtitle && <div style={{ color: C.textSoft }} className="text-xs mt-0.5">{subtitle}</div>}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {right}
          <ChevronDown size={16} color={C.textSoft} style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform 0.15s" }} />
        </div>
      </button>
