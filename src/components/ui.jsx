...
export function StatCard({ label, value, icon: Icon, tone = "cyan" }) {
  const t = TONES[tone] || TONES.cyan;
  const isInk = tone === "ink" || !!C.tileBg;
  const bg = C.tileBg || (isInk ? C.ink : t.tint);
  const textDeep = C.tileText || (isInk ? "#FFFFFF" : t.deep);
  const labelColor = C.tileText || (isInk ? "rgba(255,255,255,0.7)" : t.deep);
  return (
    <div
      style={{ background: bg, border: (isInk || C.tileBg) ? "none" : `1.5px solid ${t.border}` }}
      className="relative overflow-hidden rounded-xl px-4 py-3.5"
    >
      <div style={{ background: `${textDeep}${Math.round(C.heroBubbleOpacity * 255).toString(16).padStart(2, "0")}` }} className="wosha-hero-bubble-1 absolute -right-5 -top-6 w-16 h-16 rounded-full" />
      <div className="relative flex items-center justify-between mb-2">
        <span style={{ color: labelColor }} className="text-xs font-medium">{label}</span>
        {Icon && <Icon size={16} color={C.tileText || (isInk ? C.amber : t.deep)} strokeWidth={1.9} />}
      </div>
      <div style={{ fontFamily: monoFont, color: textDeep, fontSize: C.tileFontSize ? `${C.tileFontSize}px` : undefined }} className={"relative font-medium break-all sm:break-normal" + (C.tileFontSize ? "" : " text-lg sm:text-[22px]")}>{value}</div>
    </div>
  );
}
