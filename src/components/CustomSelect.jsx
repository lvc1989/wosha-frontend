...
export default function CustomSelect({ value, onChange, options, placeholder = "Choose…", required, style, className }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span style={{ color: C.textSoft }} className="ml-2 shrink-0">{open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</span>
      </button>
        ...
        <div style={{ background: "#fff", border: `1px solid ${C.border}`, boxShadow: "0 8px 24px rgba(11,27,51,0.15)" }} className="absolute left-0 right-0 top-full mt-1 rounded-lg overflow-hidden z-50 max-h-64 overflow-y-auto">
          {options.map((o) => (
            <button
              ...
              style={{ background: o.value === value ? "#EEF2FF" : "transparent", color: C.ink }}
