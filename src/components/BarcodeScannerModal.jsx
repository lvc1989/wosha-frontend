    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.6)" }}>
      <div style={{ background: "#fff" }} className="w-full max-w-sm rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={close} style={{ color: C.textSoft }}>✕</button>
        </div>

        {!cameraOn && (
          <button onClick={enableCamera} style={{ background: C.cyan }} className="w-full text-white rounded-lg py-2.5 text-sm font-semibold mb-4">📷 Enable Camera</button>
        )}
