...
export default function PerformanceTab() {
  ...
  return (
    <div>
      ...
      {isMobile ? (
        <div style={{ background: C.tileBg || C.cyan }} className="relative overflow-hidden rounded-2xl mb-3">
          <div style={{ background: `${C.tileText || "#FFFFFF"}${Math.round(C.heroBubbleOpacity * 255).toString(16).padStart(2, "0")}` }} className="wosha-hero-bubble-1 absolute -right-6 -top-8 w-28 h-28 rounded-full" />
          <div className="relative flex items-center justify-between px-4 py-3.5" style={{ borderBottom: `1px solid ${C.tileText ? C.tileText + "30" : "rgba(255,255,255,0.2)"}` }}>
            <span style={{ color: C.tileText || "#fff" }} className="text-xs font-medium">Gross Profit</span>
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: C.tileText || "#fff", fontSize: C.tileFontSize ? `${C.tileFontSize}px` : undefined }} className="text-lg font-semibold break-all">{money(grossProfit)}</span>
          </div>
          <div className="relative flex items-center justify-between px-4 py-3.5">
            <span style={{ color: C.tileText || "#fff" }} className="text-xs font-medium">Expense</span>
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: C.tileText || "#fff", fontSize: C.tileFontSize ? `${C.tileFontSize}px` : undefined }} className="text-lg font-semibold break-all">{money(totalExpense)}</span>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div style={{ background: C.tileBg || "#E6F1FB" }} className="relative overflow-hidden rounded-2xl p-4 text-center">
            <div style={{ background: C.tileBg ? `${C.tileText || "#FFFFFF"}${Math.round(C.heroBubbleOpacity * 255).toString(16).padStart(2, "0")}` : `rgba(24,95,165,${C.heroBubbleOpacity})` }} className="wosha-hero-bubble-1 absolute -right-8 -top-10 w-28 h-28 rounded-full" />
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", color: C.tileText || "#185FA5", fontSize: C.tileFontSize ? `${C.tileFontSize}px` : undefined }} className={"relative font-semibold break-all sm:break-normal" + (C.tileFontSize ? "" : " text-lg sm:text-2xl")}>{money(grossProfit)}</div>
            <div style={{ color: C.tileText || "#185FA5" }} className="relative text-xs font-medium mt-1">Gross Profit</div>
          </div>
          <div style={{ background: C.tileBg || "#FDE8E7" }} className="relative overflow-hidden rounded-2xl p-4 text-center">
            <div style={{ background: C.tileBg ? `${C.tileText || "#FFFFFF"}${Math.round(C.heroBubbleOpacity * 255).toString(16).padStart(2, "0")}` : `rgba(220,38,38,${C.heroBubbleOpacity})` }} className="wosha-hero-bubble-2 absolute -right-8 -top-10 w-28 h-28 rounded-full" />
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", color: C.tileText || C.danger, fontSize: C.tileFontSize ? `${C.tileFontSize}px` : undefined }} className={"relative font-semibold break-all sm:break-normal" + (C.tileFontSize ? "" : " text-lg sm:text-2xl")}>{money(totalExpense)}</div>
            <div style={{ color: C.tileText || C.danger }} className="relative text-xs font-medium mt-1">Expense</div>
          </div>
        </div>
      )}
        ...
        <table className="w-full text-sm" style={{ borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <tbody>
            {revenueByLocation.map((l, i) => (
              <tr key={l.name} style={{ borderTop: i === 0 ? "none" : "1px solid " + C.border }}>
                <td className="px-4 py-2.5" style={{ color: C.ink, wordBreak: "break-word" }}>{l.name}</td>
                <td className="px-2 py-2.5 text-right text-xs sm:text-sm" style={{ color: "#185FA5", fontFamily: "'IBM Plex Mono', monospace", wordBreak: "break-all" }}>+{money(l.revenue)}</td>
                <td className="px-4 py-2.5 text-right text-xs sm:text-sm" style={{ color: C.danger, fontFamily: "'IBM Plex Mono', monospace", wordBreak: "break-all" }}>-{money(l.expenses)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <div style={{ background: "#fff" }} className="wosha-printable w-full max-w-sm rounded-xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-2 mb-4">
              {categoryMix.map((c) => (
                <div key={c.name} className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5 text-sm">
