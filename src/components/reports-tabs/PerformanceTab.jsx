    ...
    <div>
      ...
      {isMobile ? (
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div style={{ background: C.tileBg || "#E6F1FB" }} className="relative overflow-hidden rounded-2xl p-4 text-center">
            <div style={{ background: C.tileBg ? `${C.tileText || "#FFFFFF"}${Math.round(C.heroBubbleOpacity * 255).toString(16).padStart(2, "0")}` : `rgba(24,95,165,${C.heroBubbleOpacity})` }} className="wosha-hero-bubble-1 absolute -right-4 -top-6 w-20 h-20 rounded-full" />
          ...
          <div style={{ background: C.tileBg || "#FDE8E7" }} className="relative overflow-hidden rounded-2xl p-4 text-center">
            <div style={{ background: C.tileBg ? `${C.tileText || "#FFFFFF"}${Math.round(C.heroBubbleOpacity * 255).toString(16).padStart(2, "0")}` : `rgba(220,38,38,${C.heroBubbleOpacity})` }} className="wosha-hero-bubble-2 absolute -right-4 -top-6 w-20 h-20 rounded-full" />
      ...
      <div className="bg-white rounded-2xl overflow-hidden mb-6">
        {revenueByLocation.map((l, i) => (
          <div key={l.name} className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5 px-4 py-2.5 text-sm" style={{ borderTop: i === 0 ? "none" : "1px solid " + C.border }}>
            <span style={{ color: C.ink }}>{l.name}</span>
            <span style={{ color: C.textSoft, fontFamily: "'IBM Plex Mono', monospace" }} className="text-right">+{money(l.revenue)} / -{money(l.expenses)}</span>
          </div>
        ))}
