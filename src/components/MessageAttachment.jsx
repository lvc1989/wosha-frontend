  ...
  if (message.downloaded) {
    return (
      <div style={{ color: "#166534" }} className="text-xs mt-1">
        ✓ Downloaded{message.saved_location_note ? ` — saved to: ${message.saved_location_note}` : ""}
      </div>
    );
  }
  ...
  return (
    <div className="mt-2">
      ...
      {!isImage && !isAudio && !isVideo && (
        <div style={{ color: C.textSoft }} className="text-xs mb-1">📎 {message.attachment_name || "Attachment"}</div>
      )}
      {!asking ? (
