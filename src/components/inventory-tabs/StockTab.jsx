...
import { StatCard, ListRow, StatusPill, Button, Modal, FieldLabel, LoadingState } from "../ui.jsx";

const BarcodeScannerModal = lazy(() => import("../BarcodeScannerModal.jsx"));

const money = (n) => "TZS " + Number(n || 0).toLocaleString();
const blank = { name: "", category: "", barcode: "", qty: "", reorderLevel: "", costPrice: "", sellPrice: "", sellable: false, locationId: "" };

// Original Inventory.jsx, plus one real fix: the add/edit form never had a
// branch field at all, so every product created through this form got saved
// with no branch assigned — meaning it could never appear when Sales filtered
// stock by a specific real branch. That's the actual root cause of "products
// don't show up to sell."
export default function StockTab() {
  ...
  const openAdd = () => { setEditing(null); setForm({ ...blank, locationId: loc !== "all" ? loc : (locations[0]?.id || "") }); setOpen(true); };
  const openEdit = (p) => {
    setEditing(p.id);
    setForm({ name: p.name, category: p.category || "", barcode: p.barcode || "", qty: p.qty, reorderLevel: p.reorder_level, costPrice: p.cost_price, sellPrice: p.sell_price || "", sellable: !!p.sellable, locationId: p.location_id || "" });
    setOpen(true);
  };
      ...
      {list.map((p) => {
        ...
        return (
          <ListRow
            ...
            title={p.name}
            subtitle={(locations.find((l) => l.id === p.location_id)?.name || "No branch set") + " · " + p.qty + " on hand · reorder at " + p.reorder_level + " · cost " + money(p.cost_price) + (p.sellable ? " · sell " + money(p.sell_price) : "")}
            trailing={
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div style={{ color: C.textSoft }} className="text-xs">{products.length} products tracked</div>
        {canEdit && <Button onClick={openAdd}><span className="flex items-center gap-1.5"><Plus size={16} />Add product</span></Button>}
      </div>
