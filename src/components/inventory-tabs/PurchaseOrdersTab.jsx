...
const PAYMENT_TERMS = ["Full payment", "Half payment", "Quarter payment", "Advance", "Pay on delivery", "Invoice", "Bank transfer", "Control number"];
...
function PODetail({ po, suppliers, load, quickAddSupplier }) {
  ...
  const sendOrderConfirmation = async () => {
    ...
    try {
      const result = await api.sendPOOrderConfirmation(po.id);
      if (!result.delivered) setConfirmationError(result.deliveryError || "Couldn't deliver the email — try again.");
      load();
    } catch (err) {
  ...
  const scanDeliveryNoteToExpense = async () => {
    setOcrBusy(true);
    try {
      const { createWorker } = await import("tesseract.js");
      const worker = await createWorker("eng");
      const imageBlob = await (await fetch(po.delivery_note_url)).blob();
      const { data } = await worker.recognize(imageBlob);
      await worker.terminate();
      const text = data.text || "";
      // A real receipt has plenty of number-like tokens (quantities, unit
      // prices, a total) — this looks for the largest one as a starting guess
      // for the total amount, since that's usually right, but it's shown for
      // confirmation rather than trusted outright, since OCR on a real
      // photographed document is genuinely less reliable than a clean scan.
      const numbers = (text.match(/[\d,]{3,}/g) || []).map((n) => Number(n.replace(/,/g, ""))).filter((n) => n > 0);
      const guessedAmount = numbers.length ? Math.max(...numbers) : "";
      const firstLine = text.split("\n").map((l) => l.trim()).find((l) => l.length > 3) || "";
      setExpenseDraft({ description: firstLine || "Delivery — " + (po.supplier_name || "Supplier"), amount: guessedAmount, rawText: text });
    } catch (err) {
      setExpenseDraft({ description: "Delivery — " + (po.supplier_name || "Supplier"), amount: "", rawText: "", ocrFailed: true });
    }
    setOcrBusy(false);
  };
  const confirmExpenseFromDelivery = async () => {
    await api.addExpense({ locationId: po.location_id, category: "Supplies", amount: Number(expenseDraft.amount), note: expenseDraft.description, purchaseOrderId: po.id });
  ...
  return (
    <div className="mt-3 pt-3" style={{ borderTop: "1px solid " + C.border }}>
      ...
      {po.status === "Approved" && po.supplier_id && (
        <div className="mb-3">
          {po.order_confirmation_sent_at
            ? <div style={{ color: C.textSoft }} className="text-xs">Order confirmation sent to {po.supplier_name} on {new Date(po.order_confirmation_sent_at).toLocaleDateString()}.</div>
            : <button onClick={sendOrderConfirmation} disabled={sendingConfirmation} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="text-xs font-semibold px-3 py-2 rounded-lg disabled:opacity-50">{sendingConfirmation ? "Sending…" : `Send order confirmation to ${po.supplier_name}`}</button>}
          {confirmationError && <div style={{ color: C.danger }} className="text-xs mt-1">{confirmationError}</div>}
        </div>
      )}
          ...
          {po.delivery_note_url
            ? (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button type="button" onClick={scanDeliveryNoteToExpense} disabled={ocrBusy} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="text-xs font-semibold px-2 py-1 rounded-lg disabled:opacity-50">{ocrBusy ? "Reading…" : "Create expense from this"}</button>
              </div>
            )
      ...
      <Modal open={!!expenseDraft} onClose={() => setExpenseDraft(null)}>
        {expenseDraft && (
          <>
            <div style={{ color: C.ink, fontFamily: "'Space Grotesk', sans-serif" }} className="text-lg font-bold mb-1">Create expense from delivery note</div>
            <div style={{ color: C.textSoft }} className="text-xs mb-4">
              {expenseDraft.ocrFailed
                ? "Couldn't read the image automatically — fill this in by hand."
                : "Read from the delivery note automatically — review and correct before saving, since this is real photographed text, not a clean scan."}
            </div>
            <FieldLabel>Description</FieldLabel>
            <input value={expenseDraft.description} onChange={(e) => setExpenseDraft({ ...expenseDraft, description: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
            <FieldLabel>Amount (TZS)</FieldLabel>
            <input type="number" value={expenseDraft.amount} onChange={(e) => setExpenseDraft({ ...expenseDraft, amount: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-4 text-sm" />
            <div className="flex gap-2">
              <Button variant="ghost" onClick={() => setExpenseDraft(null)} className="flex-1">Cancel</Button>
              <Button onClick={confirmExpenseFromDelivery} disabled={!expenseDraft.amount} className="flex-1">Save expense</Button>
            </div>
          </>
        )}
      </Modal>
