      ...
      <div className="wosha-fixed-topstrip">
        ...
        <div style={{ background: brand?.theme_topbar_color || "#fff" }} className="flex-1 flex items-center justify-end gap-2 sm:gap-3 px-3 sm:px-6">
          ...
          <div ref={langRef} className="relative">
            <button onClick={() => setLangOpen((v) => !v)} title="Language" style={{ color: topbarIsLight ? C.textSoft : "#fff" }} className="flex items-center gap-1">
              <Globe size={19} strokeWidth={1.75} />
            </button>
            {langOpen && (
              <div style={{ background: "#fff", border: `1px solid ${C.border}`, boxShadow: "0 8px 24px rgba(11,27,51,0.15)", minWidth: 140 }} className="absolute right-0 top-8 rounded-xl overflow-hidden z-50">
                {LANGUAGES.map((l) => (
                  <button
                    key={l.code}
          ...
          <div ref={bellRef} className="relative">
            <button onClick={() => setBellOpen((v) => !v)} className="relative" style={{ color: topbarIsLight ? C.textSoft : "#fff" }}>
              ...
              {reminders.length > 0 && <span style={{ background: C.amberDeep }} className="absolute -top-1.5 -right-1.5 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{reminders.length}</span>}
            </button>
            {bellOpen && (
