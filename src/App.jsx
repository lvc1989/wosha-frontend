...
import {
  ...
  LineChart, Bell, ChevronDown, ChevronUp, Menu, X,
} from "lucide-react";
...
function Shell({ user, onLogout, onUserUpdate, children }) {
  ...
  const layoutStyle = user.layoutStyle || "sidebar";
  const isHamburger = layoutStyle === "grid-hamburger";
  const isGridSidebar = layoutStyle === "grid";
  ...
  return (
    <div className="wosha-app-shell" style={{ background: C.bg, fontFamily: bodyFont, color: C.text }}>
      <style>{`
        ...
        .wosha-fixed-main { position: fixed; top: 60px; left: ${isHamburger ? "0" : "230px"}; right: 0; bottom: 0; overflow-y: auto; overflow-x: hidden; z-index: 40; }
        .wosha-bell-dropdown { position: fixed; top: 60px; right: 16px; width: min(320px, calc(100vw - 246px)); }
        @media (max-width: 767px) {
          .wosha-bell-dropdown { width: min(320px, calc(100vw - 92px)); }
        }
        .wosha-logo-strip { width: 230px; }
        .wosha-nav-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; }
        .wosha-nav-grid .wosha-navlink { flex-direction: column; padding: 10px 4px !important; text-align: center; gap: 4px !important; }
        .wosha-nav-grid .wosha-navlabel { font-size: 10px; line-height: 1.15; }
        .wosha-drawer-overlay { position: fixed; inset: 0; z-index: 60; background: rgba(11,27,51,0.5); display: flex; }
        .wosha-drawer-panel { width: min(320px, 82vw); height: 100%; overflow-y: auto; padding: 1rem; }
        @media (max-width: 767px) {
          .wosha-fixed-sidebar { width: 64px !important; }
          .wosha-fixed-main { left: ${isHamburger ? "0" : "64px"} !important; padding: 0.75rem !important; }
          ...
          .wosha-app-shell .fixed.inset-0 { left: ${isHamburger ? "0" : "64px"} !important; }
        }
      ...
      <div className="wosha-fixed-topstrip">
        {isHamburger && (
          <button onClick={() => setDrawerOpen(true)} className="wosha-logo-strip flex items-center justify-center shrink-0" style={{ background: brand?.sidebar_color || C.ink, width: 60 }} aria-label="Open menu">
            <Menu size={22} color="#fff" strokeWidth={1.75} />
          </button>
        )}
        <button onClick={() => navigate("/")} style={{ background: brand?.sidebar_color || C.ink }} className={`${isHamburger ? "flex items-center gap-2 px-4 shrink-0 text-left" : "wosha-logo-strip flex items-center gap-2 px-4 shrink-0 text-left"}`}>
          {brand?.logo_url ? (
      ...
      {!isHamburger && (
        <aside style={{ background: brand?.sidebar_color || C.ink }} className="wosha-fixed-sidebar flex flex-col py-4 px-2">
          <nav className={isGridSidebar ? "wosha-nav-grid" : "flex flex-col gap-1"}>
            {nav.map((n) => (
      ...
      {isHamburger && drawerOpen && (
        <div className="wosha-drawer-overlay" onClick={() => setDrawerOpen(false)}>
          <div style={{ background: brand?.sidebar_color || C.ink }} className="wosha-drawer-panel" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4 px-1">
              <span style={{ color: "#fff" }} className="text-sm font-semibold">Menu</span>
              <button onClick={() => setDrawerOpen(false)} style={{ color: "rgba(255,255,255,0.65)" }} aria-label="Close menu"><X size={20} /></button>
            </div>
            <nav className="wosha-nav-grid">
              {nav.map((n) => (
                <Link key={n.to} to={n.to} title={n.label}
                  style={{ background: location.pathname === n.to ? "rgba(255,255,255,0.1)" : "transparent", color: location.pathname === n.to ? "#fff" : "rgba(255,255,255,0.65)" }}
                  className="wosha-navlink px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
