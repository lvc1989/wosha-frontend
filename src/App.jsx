...
import { applyTheme, applyHeroStyle, applyTileStyle } from "./components/ui.jsx";
      ...
      <div className="wosha-fixed-topstrip">
        ...
        <div style={{ background: brand?.theme_topbar_color || "#fff" }} className="flex-1 flex items-center justify-end gap-2 sm:gap-3 px-3 sm:px-6">
          ...
          <div ref={bellRef} className="relative">
            <button onClick={() => setBellOpen((v) => !v)} className="relative flex items-center justify-center w-8 h-8 rounded-lg" style={{ color: topbarIsLight ? C.textSoft : "#fff", background: topbarIsLight ? "rgba(11,27,51,0.05)" : "rgba(255,255,255,0.15)" }}>
              <Bell size={18} strokeWidth={1.75} />
              {reminders.length > 0 && <span style={{ background: C.danger }} className="absolute -top-1 -right-1 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{reminders.length}</span>}
            </button>
            {bellOpen && (
