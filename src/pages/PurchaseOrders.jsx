...
import { PageHeader, StatusPill, Button, Modal, FieldLabel, EmptyState, HeroCard, LoadingState } from "../components/ui.jsx";
    ...
    <div className="mt-3 pt-3" style={{ borderTop: "1px solid " + C.border }}>
      ...
      {sendResult && (
        <div className="mb-3 flex flex-col gap-1">
          {sendResult.map((r) => (
            <div key={r.supplierId} style={{ color: r.delivered ? "#3B6D11" : C.danger }} className="text-xs">{r.supplierName || r.supplierId}: {r.delivered ? "sent" : r.reason}</div>
          ))}
        </div>
      )}
        ...
        {po.items.map((it) => {
          ...
          return (
            <div key={it.id} className="flex items-center justify-between gap-2 flex-wrap text-xs">
              <span style={{ color: done ? "#3B6D11" : C.ink }} className="flex items-center gap-1">{done ? <CheckCircle2 size={13} /> : <Circle size={13} />} {it.name} ({it.received_qty || 0}/{it.qty})</span>
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={pos.length + " total"}
        action={<Button onClick={() => setOpen(true)}><span className="flex items-center gap-1.5"><Plus size={16} />New purchase order</span></Button>}
      />

      {pos.length === 0 ? (
        <div className="bg-white rounded-xl"><EmptyState icon={ClipboardList} title="No purchase orders yet" body="Create one to start requesting quotations from suppliers." action={<Button onClick={() => setOpen(true)}>New purchase order</Button>} /></div>
      ) : (
        <div className="flex flex-col gap-2">
          {pos.map((po) => (
            <div key={po.id} className="bg-white rounded-xl px-5 py-4" style={{ borderLeft: "4px solid " + (po.status === "Pending Approval" ? "#EF9F27" : "#2B6CF6") }}>
