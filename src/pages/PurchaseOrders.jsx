...
function PODetail({ po, suppliers, load, quickAddSupplier }) {
  ...
  return (
    <div className="mt-3 pt-3" style={{ borderTop: `1px solid ${C.border}` }}>
      ...
      <select value={po.supplier_id || ""} onChange={(e) => assignAwarded(e.target.value)} style={{ borderColor: C.border }} className="border rounded-lg px-2 py-1.5 text-xs mb-3">
        <option value="">Not yet decided</option>
        {shortlisted.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
      </select>

      <div style={{ color: C.textSoft }} className="text-xs font-semibold uppercase mb-1">Payment Terms</div>
      <select value={po.payment_terms} onChange={(e) => setPaymentTerms(e.target.value)} style={{ borderColor: C.border }} className="border rounded-lg px-2 py-1.5 text-xs mb-3">
        ...
        {po.items.map((it) => {
          ...
          return (
            <div key={it.id} className="flex items-center justify-between gap-2 text-xs">
              <span style={{ color: done ? "#166534" : C.ink }}>{done ? "✓" : "○"} {it.name} ({it.received_qty || 0}/{it.qty})</span>
              {!done && <button type="button" onClick={() => setScanningItemId(it.id)} style={{ color: C.cyan }} className="font-semibold">📷 Scan</button>}
            </div>
          );
        })}
      </div>
      ...
      {scanningItemId && <BarcodeScannerModal onDetected={scanReceive} onClose={() => setScanningItemId(null)} />}
    </div>
  );
}
            ...
            {poCategories.length > 0 && (
              <>
                ...
                <div className="flex gap-2 mb-2 overflow-x-auto pb-1">
                  {poCategories.map((c) => (
                    <button key={c.id} type="button" onClick={() => setSelectedCategory(c.name)} style={{ background: selectedCategory === c.name ? C.cyan : "#fff", color: selectedCategory === c.name ? "#fff" : C.ink, border: `1px solid ${C.border}` }} className="text-xs font-semibold px-3 py-1.5 rounded-full whitespace-nowrap shrink-0">{c.name}</button>
                  ))}
                </div>
            ...
            {items.map((it, i) => (
              <div key={i} className="flex flex-col sm:flex-row gap-2 mb-2 pb-2 sm:pb-0" style={{ borderBottom: "1px solid #F1F2F4" }}>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => removeRow(i)} style={{ color: C.danger }} className="text-sm px-2">✕</button>
                </div>
              </div>
            ))}
