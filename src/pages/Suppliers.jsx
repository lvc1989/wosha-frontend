            ...
            <div className="flex flex-col gap-2 mb-4">
              {(payments[payTarget.id] || []).map((p) => (
                <div key={p.id} style={{ background: "#F5F7FA" }} className="rounded-lg p-3">
                  ...
                  {p.downloaded ? (
                    <div style={{ color: "#166534" }} className="text-xs flex items-center gap-1"><Check size={12} /> Receipt downloaded{p.saved_location_note ? ` — saved to: ${p.saved_location_note}` : ""}</div>
                  ) : p.receipt_url ? (
