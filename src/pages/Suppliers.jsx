...
export default function Suppliers() {
  ...
  return (
    <div>
      <PageHeader
        title="Suppliers"
        subtitle={suppliers.length + " on file"}
        action={canEdit && <Button onClick={openAdd}><span className="flex items-center gap-1.5"><Plus size={16} />Add supplier</span></Button>}
      />

      {suppliers.length === 0 ? (
        <div className="bg-white rounded-xl"><EmptyState icon={Truck} title="No suppliers yet" body="Add one so purchase orders can request quotations." action={canEdit && <Button onClick={openAdd}>Add supplier</Button>} /></div>
      ) : (
          ...
          <>
            ...
            <div className="flex flex-col gap-2 mb-4">
              {(payments[payTarget.id] || []).map((p) => (
                <div key={p.id} style={{ background: "#F5F7FA" }} className="rounded-lg p-3">
                  ...
                  {p.downloaded ? (
                    <div style={{ color: "#3B6D11" }} className="text-xs flex items-center gap-1"><Check size={12} /> Receipt downloaded{p.saved_location_note ? " — saved to: " + p.saved_location_note : ""}</div>
                  ) : p.receipt_url ? (
            ...
            <form onSubmit={submitPay} style={{ borderTop: "1px solid " + C.border }} className="pt-4">
              ...
              <div className="mb-3">
                ...
                {payForm.receiptUrl && <div style={{ color: "#3B6D11" }} className="text-xs mt-1">Attached: {payForm.receiptName}</div>}
              </div>
              <div className="flex gap-2">
