          ...
          return (
            <div key={b.id} className="flex items-center justify-between gap-2 flex-wrap px-5 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.border}` }}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isFinished && <button onClick={async () => { await api.archiveBooking(b.id); setBookings((prev) => prev.filter((x) => x.id !== b.id)); }} className="text-xs font-semibold" style={{ color: C.textSoft }}>Remove</button>}
              </div>
            </div>
          );
        })}
