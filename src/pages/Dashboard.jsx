...
import { useBranch, useUser, useBrand, C } from "../App.jsx";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatCard label="Bookings" value={bookings.length} icon={Calendar} tone="cyan" tileKey="bookings" />
        <StatCard label="Revenue collected" value={money(revenue)} icon={Wallet} tone="ink" tileKey="revenue" />
        <StatCard label="Active jobs" value={activeJobs} icon={ClipboardCheck} tone="success" tileKey="activeJobs" />
        <StatCard label="Expenses approved" value={money(expTotal)} icon={AlertTriangle} tone="amber" tileKey="expenses" />
      </div>
      ...
      <div className="mb-6">
        <QuickActionGrid
          ...
          tileBg={brand?.menu_icon_bg_color}
          iconColor={brand?.menu_icon_color}
        />
      </div>
      ...
      <div className="flex items-center justify-between mb-2">
        <div style={{ color: C.ink }} className="text-sm font-semibold">Today's bookings</div>
        {bookings.length > 4 && <button onClick={() => navigate("/bookings")} className="text-xs font-semibold" style={{ color: C.cyan }}>View all →</button>}
      </div>
      {bookings.length === 0 ? (
