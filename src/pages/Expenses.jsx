...
export default function Expenses() {
  ...
  const [form, setForm] = useState({ category: "Utilities", amount: "", note: "" });
  const [open, setOpen] = useState(false);

  const load = () => api.getExpenses().then(setExpenses).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);

  const submit = async (e) => { e.preventDefault(); await api.addExpense({ ...form, amount: Number(form.amount) }); setForm({ category: "Utilities", amount: "", note: "" }); setOpen(false); load(); };
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <form onSubmit={submit} style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            ...
            <input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
