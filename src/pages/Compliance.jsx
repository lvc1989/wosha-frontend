  ...
  return (
    <div className="px-5 py-3" style={{ borderTop: "1px solid " + C.border }}>
      ...
      {t.status === "Submitted" && t.attachment_url && (
        <a href={t.attachment_url} target="_blank" rel="noreferrer" style={{ color: C.cyan }} className="text-xs underline mt-1 block">📎 {t.attachment_name || "View submitted file"}</a>
      )}
      ...
      {reviewOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <div style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            ...
            {t.attachment_url && <a href={t.attachment_url} target="_blank" rel="noreferrer" style={{ color: C.cyan }} className="text-xs underline block mb-3">📎 {t.attachment_name || "View submitted file"}</a>}
  ...
  return (
    <div>
      ...
      <div className="flex justify-end mb-2">
        <button onClick={() => window.print()} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="text-xs font-semibold px-3 py-1.5 rounded-lg">🖨 Print Filings</button>
      </div>
          ...
          <form onSubmit={handleAssignTask} style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            ...
            {templates.length > 0 && (
              <>
                ...
                <select value="" onChange={(e) => e.target.value && setTaskForm({ ...taskForm, title: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm">
                  <option value="">Select a template (or type your own below)…</option>
      ...
      {taxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <form onSubmit={handleAddTax} style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            ...
            <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Due date</label>
            <input required type="date" value={taxForm.dueDate} onChange={(e) => setTaxForm({ ...taxForm, dueDate: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-4 text-sm" />
            <div className="flex gap-2">
