...
function TaskRow({ t, canReview, load }) {
  ...
  const isOverdue = t.due_date && !["Done"].includes(t.status) && new Date(t.due_date) < new Date(new Date().toDateString());
    ...
    <div className="px-5 py-3" style={{ borderTop: "1px solid " + C.border }}>
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          ...
          <div style={{ color: isOverdue ? C.danger : C.textSoft }} className="text-xs font-medium">{t.assigned_to_name || "Unassigned"} · {isOverdue ? "overdue since " : "due "}{t.due_date ? new Date(t.due_date).toLocaleDateString() : "no date"}</div>
        </div>
  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff", border: "1px solid " + C.border }} className="wosha-printable rounded-xl overflow-hidden mb-6">
        ...
        {taxItems.map((t, i) => {
          const isOverdue = t.status !== "Filed" && new Date(t.due_date) < new Date(new Date().toDateString());
          return (
          <div key={t.id} className="flex items-center justify-between gap-2 flex-wrap px-5 py-3" style={{ borderTop: i === 0 ? "none" : "1px solid " + C.border }}>
            <div>
              ...
              <div style={{ color: isOverdue ? C.danger : C.textSoft }} className="text-xs font-medium">{isOverdue ? "Overdue since " : "Due "}{new Date(t.due_date).toLocaleDateString()}</div>
            </div>
            {t.status === "Filed" ? (
