...
export default function Sales() {
  ...
  // A sale always happens at one physical branch, and each branch has its own
  // stock — but "All Branches" being selected at the top used to make this page
  // silently pick whichever branch happened to load first, with zero indication
  // of which one, or why its product list might be empty. This makes that choice
  // explicit instead of hidden.
  const [saleBranchId, setSaleBranchId] = useState(loc !== "all" ? loc : "");
  useEffect(() => { if (loc !== "all") setSaleBranchId(loc); }, [loc]);
  const activeBranchId = saleBranchId || branchId;
  ...
  return (
    <div>
      <PageHeader title="Record sale" subtitle={"Selling from " + (activeBranchName || "—")} action={loc === "all" && <button onClick={() => setSaleBranchId("")} style={{ color: "#fff", border: "1px solid rgba(255,255,255,0.3)" }} className="text-xs font-semibold px-3 py-1.5 rounded-lg">Switch branch</button>} />
      <div className="flex items-center gap-1.5 mb-4 -mt-2">
        <span style={{ background: "#639922" }} className="w-1.5 h-1.5 rounded-full animate-pulse" />
        <span style={{ color: C.textSoft }} className="text-xs">Hardware barcode scanner ready — scan anytime, it adds straight to the basket</span>
      </div>
      {done && (
