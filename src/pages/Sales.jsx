...
const C = { ink: "#0B1B33", cyan: "#2B6CF6", border: "#E4E7EC", textSoft: "#667085", successBg: "#E6F4EA", amberBg: "#FEF3C7", amberDeep: "#92400E", danger: "#DC2626" };
...
export default function Sales() {
  ...
  useEffect(() => {
    Promise.all([api.getServices(), api.getProducts(branchId), api.getCustomers()])
  ...
  const scanToAdd = async (code) => {
    ...
    if (scanMode === "plate") {
      setPlate(code);
      try {
        const cust = await api.getCustomerByPlate(code);
        setCustomerId(cust.id);
        setWalkIn("");
        setScanMsg(`Recognized returning customer: ${cust.name}`);
      } catch {
        setScanMsg(`Plate/reference "${code}" recorded — no matching customer on file, sale will be identified by this alone.`);
      }
      setTimeout(() => setScanMsg(""), 5000);
      return;
    }

    // Product mode: try a real product first, then fall back to a payment code —
    // a payment code's own barcode can also be scanned here to quickly add whatever
    // service/product it's linked to, same as scanning the product directly.
    try {
  ...
  const finalizeSale = async (method) => {
    ...
    const booking = await api.addBooking({ locationId: branchId, customerId: finalCustomerId, vehiclePlate: plate || "Walk-in / Counter Sale", serviceIds });

    const isInvoiceOnly = method === "Invoice (Company)";
    const invoice = await api.createInvoice({
      bookingId: booking.id, locationId: branchId, items,
  ...
  return (
    <div>
      ...
      {done && (
        <div style={{ background: C.successBg, color: "#166534" }} className="rounded-lg px-4 py-3 text-sm mb-6 flex items-center justify-between gap-2 flex-wrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2">
          ...
          {scanMsg && <div style={{ background: C.amberBg, color: C.amberDeep }} className="rounded-lg px-3 py-2 text-xs mb-3">{scanMsg}</div>}
        ...
        <div>
          ...
          <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-4">
            {!companyMode ? (
              <>
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Customer (optional — pick, type, or just scan the plate below)</label>
                <div className="mb-2">
                ...
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Or walk-in name (optional)</label>
                ...
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Vehicle plate / reference ID (optional)</label>
                <div className="flex gap-2 mb-1">
                  <input value={plate} onChange={(e) => setPlate(e.target.value)} placeholder="e.g. T 123 ABC" style={{ borderColor: C.border }} className="flex-1 border rounded-lg px-3 py-2 text-sm" />
              ...
              <div style={{ background: C.amberBg, color: C.amberDeep }} className="rounded-lg px-3 py-2 text-xs mb-3">Billing to company — fill in company details at checkout.</div>
            )}
                    ...
                    <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td className="py-1.5" style={{ color: C.ink }}>
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span style={{ fontFamily: "monospace" }}>{b.qty}</span>
                          <button onClick={() => changeQty(i, 1)} style={{ border: `1px solid ${C.border}` }} className="w-5 h-5 rounded text-xs">+</button>
                        </div>
                      </td>
                      <td className="py-1.5 text-right" style={{ color: C.textSoft, fontFamily: "monospace" }}>{b.rate.toLocaleString()}</td>
                      <td className="py-1.5 text-right" style={{ color: C.ink, fontFamily: "monospace" }}>{(b.rate * b.qty).toLocaleString()}</td>
            ...
            <div style={{ borderTop: `1px solid ${C.border}` }} className="pt-2 flex items-center justify-between mb-3">
              ...
              <span style={{ color: C.ink, fontFamily: "monospace" }} className="text-sm font-bold">{money(total)}</span>
            </div>
            ...
            <button onClick={() => setPayOpen(true)} disabled={!basket.length} style={{ background: C.cyan }} className="w-full text-white text-sm font-semibold py-2.5 rounded-lg disabled:opacity-40">Checkout</button>
          </div>
        </div>
      </div>
      ...
      {payOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <div style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            <div style={{ color: C.ink }} className="text-lg font-bold mb-4">Choose Payment</div>
            {companyMode ? (
              <>
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Company name</label>
                ...
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Company TIN</label>
                ...
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Company address</label>
                ...
                <button onClick={() => finalizeSale("Invoice (Company)")} disabled={!companyForm.companyName.trim()} style={{ background: C.cyan }} className="w-full text-white text-sm font-semibold py-2.5 rounded-lg disabled:opacity-40">Generate Invoice</button>
              </>
            ) : (
              <div className="flex flex-col gap-2">
                {["Cash", "Control Number", "Mobile Money"].map((m) => (
                  <button key={m} onClick={() => finalizeSale(m)} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="py-2.5 rounded-lg text-sm font-semibold">{m}</button>
                ))}
              </div>
            )}
