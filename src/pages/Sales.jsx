...
export default function Sales() {
  ...
  useEffect(() => {
    Promise.all([api.getServices(), api.getProducts(branchId), api.getCustomers()])
      .then(([s, p, c]) => { setServices(s); setProducts(p.filter((x) => x.sellable)); setCustomers(c); if (c.length && !customerId) setCustomerId(c[0].id); })
  ...
  const scanToAdd = async (code) => {
    ...
    try {
      ...
      setScanMsg("No sellable product matches that barcode at this branch.");
    }
          ...
          <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-4">
            {!companyMode ? (
              <>
                ...
                <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-2 text-sm">
                ...
                <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Vehicle plate (optional)</label>
                <input value={plate} onChange={(e) => setPlate(e.target.value)} placeholder="e.g. T 123 ABC" style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
              </>
            ) : (
                ...
                <tbody>
                  ...
                  {basket.map((b, i) => (
                    <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td className="py-1.5 text-right"><button onClick={() => removeItem(i)} style={{ color: C.textSoft }}>✕</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
