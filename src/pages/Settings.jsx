...
import { Check, Settings2, ShieldCheck, X, Star, Globe, Image as ImageIcon, Megaphone } from "lucide-react";
...
export default function Settings() {
  ...
  const [adBannerForm, setAdBannerForm] = useState({ title: "", description: "", imageUrl: "", linkText: "" });
  ...
  const reloadAdBanners = () => api.getAdBanners().then(setAdBanners);
  const submitAdBanner = async (e) => {
    e.preventDefault();
    if (!adBannerForm.title.trim() || !adBannerForm.imageUrl) return;
    await api.addAdBanner(adBannerForm);
    setAdBannerForm({ title: "", description: "", imageUrl: "", linkText: "" });
    setAdBannerOpen(false);
    reloadAdBanners();
  };
  const toggleAdBannerVisible = async (id) => { await api.toggleAdBanner(id); reloadAdBanners(); };
  const removeAdBannerItem = async (id) => { if (confirm("Remove this ad banner?")) { await api.removeAdBanner(id); reloadAdBanners(); } };
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl p-5 mb-4">
        <div className="flex items-center justify-between mb-3">
          <div style={{ color: C.ink }} className="text-sm font-semibold flex items-center gap-1.5"><Megaphone size={15} color="#A32D2D" /> Promotions &amp; ad banners</div>
          <button onClick={() => setAdBannerOpen(true)} className="text-xs font-semibold" style={{ color: C.cyan }}>+ Add banner</button>
        </div>
        <div style={{ color: C.textSoft }} className="text-xs mb-3">Special offers or seasonal promotions shown prominently on the website — separate from the work-photo gallery below.</div>
        {adBanners.length === 0 && <div style={{ color: C.textSoft }} className="text-xs">None yet — add one to promote a current offer.</div>}
        <div className="flex flex-col gap-2">
          {adBanners.map((b) => (
            <div key={b.id} style={{ background: "#F5F7FA", opacity: b.visible ? 1 : 0.5 }} className="rounded-lg p-3 flex items-center gap-3">
              <img src={b.image_url} alt="" className="w-12 h-12 rounded-lg object-cover shrink-0" />
              <div className="flex-1 min-w-0">
                <div style={{ color: C.ink }} className="text-xs font-semibold truncate">{b.title}{b.visible === false ? " (hidden)" : ""}</div>
                {b.description && <div style={{ color: C.textSoft }} className="text-xs truncate">{b.description}</div>}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => toggleAdBannerVisible(b.id)} className="text-xs font-semibold" style={{ color: C.cyan }}>{b.visible ? "Hide" : "Show"}</button>
                <button onClick={() => removeAdBannerItem(b.id)} style={{ color: C.danger }}><X size={13} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={adBannerOpen} onClose={() => setAdBannerOpen(false)}>
        <form onSubmit={submitAdBanner}>
          <div style={{ color: C.ink, fontFamily: "'Space Grotesk', sans-serif" }} className="text-lg font-bold mb-4">Add promotion / ad banner</div>
          <FieldLabel>Title</FieldLabel>
          <input required value={adBannerForm.title} onChange={(e) => setAdBannerForm({ ...adBannerForm, title: e.target.value })} placeholder="e.g. Weekend Special — 20% Off" style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
          <FieldLabel>Description (optional)</FieldLabel>
          <textarea rows={2} value={adBannerForm.description} onChange={(e) => setAdBannerForm({ ...adBannerForm, description: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
          <FieldLabel>Button text (optional — defaults to "Book Now")</FieldLabel>
          <input value={adBannerForm.linkText} onChange={(e) => setAdBannerForm({ ...adBannerForm, linkText: e.target.value })} placeholder="e.g. Claim Offer" style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
          <FieldLabel>Image</FieldLabel>
          <div className="mb-4">
            <FileDropzone accept="image/*" maxSizeMb={10} label="Drop a promotional image here" onUploaded={(f) => setAdBannerForm({ ...adBannerForm, imageUrl: f.url })} />
            {adBannerForm.imageUrl && <div style={{ color: "#185FA5" }} className="text-xs mt-1">Attached — ready to add.</div>}
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="ghost" onClick={() => setAdBannerOpen(false)} className="flex-1">Cancel</Button>
            <Button type="submit" disabled={!adBannerForm.imageUrl} className="flex-1">Add</Button>
          </div>
        </form>
      </Modal>
