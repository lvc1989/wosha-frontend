  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl p-6 mb-6 max-w-lg">
        ...
        <CollapsibleSection tone="blue" title="Sidebar & logo strip color" subtitle="Changes everywhere the sidebar and top-left logo strip appear, for everyone who logs in.">
      ...
      <CollapsibleSection tone="blue" title="Loading screen color" subtitle="Shown for a moment every time the app opens, before it knows who's logged in.">
      ...
      <CollapsibleSection tone="yellow" title="App theme colors" subtitle="Changes every button, link, and highlight across the whole app immediately. Status colors (approved/pending/rejected, low stock) never change.">
      ...
      <CollapsibleSection tone="blue" title="Top bar color" subtitle="The strip along the top — logo, branch picker, notifications, account. Text and icons automatically switch to stay readable, whatever color is picked.">
        <div className="flex items-center gap-2 flex-wrap">
      ...
      <CollapsibleSection tone="yellow" title="Big banner &amp; tile style" subtitle="The large dark bands at the top of every page, and similar big tiles like Reports' profit/loss figures — background color, text color, and how visible the softly-drifting background circles are behind the text.">
        <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Background color</label>
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <input type="color" value={settings.hero_bg_color || "#0B1B33"} onChange={async (e) => { await api.updateSettings({ heroBgColor: e.target.value }); load(); }} className="w-8 h-8 rounded-full border-0 cursor-pointer" style={{ padding: 0 }} />
        </div>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <input type="color" value={settings.hero_text_color || "#FFFFFF"} onChange={async (e) => { await api.updateSettings({ heroTextColor: e.target.value }); load(); }} className="w-8 h-8 rounded-full border-0 cursor-pointer" style={{ padding: 0 }} />
        </div>
        ...
        <div style={{ borderTop: `1px solid ${C.border}` }} className="pt-4">
          ...
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <input type="color" value={settings.tile_bg_color || "#FFC93C"} onChange={async (e) => { await api.updateSettings({ tileBgColor: e.target.value, tileTextColor: settings.tile_text_color || "#0B1B33" }); load(); }} className="w-8 h-8 rounded-full border-0 cursor-pointer" style={{ padding: 0 }} />
            ...
            return (
              <>
                ...
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  <input type="color" value={current.bg || "#2B6CF6"} onChange={(e) => saveTile({ bg: e.target.value })} className="w-8 h-8 rounded-full border-0 cursor-pointer" style={{ padding: 0 }} />
                </div>
      ...
      <CollapsibleSection tone="blue" title="Icon size &amp; colors" subtitle="Applies to the Home page's quick actions and the grid Menu page. Size is per-device — try one and see how it feels. Colors apply for everyone.">
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <input type="color" value={settings.menu_icon_bg_color || "#E6F1FB"} onChange={async (e) => { await api.updateSettings({ menuIconBgColor: e.target.value }); load(); }} className="w-8 h-8 rounded-full border-0 cursor-pointer" style={{ padding: 0 }} />
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <input type="color" value={settings.menu_icon_color || "#185FA5"} onChange={async (e) => { await api.updateSettings({ menuIconColor: e.target.value }); load(); }} className="w-8 h-8 rounded-full border-0 cursor-pointer" style={{ padding: 0 }} />
        </div>
      </CollapsibleSection>

      <CollapsibleSection tone="yellow" title="App icon background color" subtitle="The color behind your logo when it's used as the installed app icon on a phone or computer. White is the safest default if your logo already has its own white background built in.">
        <div className="flex items-center gap-2 flex-wrap">
      ...
      {user?.isPrimaryOwner && (
        <CollapsibleSection tone="yellow" title="Printing Design" subtitle="The header and footer shown automatically on every printed invoice, receipt, and report — sized to stay compact and leave the printable area balanced.">
      ...
      <CollapsibleSection tone="blue" title="Printer Profiles" subtitle="Set up each printer once — paper size, how it connects, and preview how a receipt will actually look on it before printing for real.">
        {printers.map((p) => (
      ...
      <CollapsibleSection tone="blue" title="Category Lists" subtitle="Every dropdown in the app (services, products, expenses, purchase orders, client segments, cash flow) pulls from these lists — add or remove options here and they show up everywhere immediately.">
      ...
      <CollapsibleSection tone="blue" title="More Category Lists" subtitle="Client segments, cash flow, task templates, and branch targets — the same idea as Category Lists above, split out since these are used less often.">
