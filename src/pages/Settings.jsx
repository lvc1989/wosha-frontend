...
export default function Settings() {
  ...
  const runTestEmail = async (e) => {
    e.preventDefault();
    setTestBusy(true); setTestEmailMsg("");
    try {
      const result = await api.testEmail(testEmailTo);
      setTestEmailMsg(result.sent ? "✓ Sent — check that inbox now." : `Not sent: ${result.reason}`);
    } catch (err) {
  ...
  const runTestSms = async (e) => {
    e.preventDefault();
    setTestBusy(true); setTestSmsMsg("");
    try {
      const result = await api.testSms(testSmsTo);
      setTestSmsMsg(result.sent ? "✓ Sent — check that phone now." : `Not sent: ${result.reason}`);
    } catch (err) {
      setTestSmsMsg(err.message);
    } finally {
      setTestBusy(false);
    }
  };
    ...
    <div>
      ...
      {(user?.role === "owner") && (
        <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-6 mb-6 max-w-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setBranchFieldsOpen(true)} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="text-xs font-semibold px-3 py-1.5 rounded-lg">⚙ Manage Branch Info Fields</button>
          </div>
          {branches.map((b) => (
      ...
      {user?.role === "owner" && notifStatus && (
        <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-6 mb-6 max-w-lg">
          ...
          {testEmailMsg && <div style={{ color: testEmailMsg.startsWith("✓") ? "#166534" : C.danger }} className="text-xs mb-2">{testEmailMsg}</div>}
          ...
          {testSmsMsg && <div style={{ color: testSmsMsg.startsWith("✓") ? "#166534" : C.danger }} className="text-xs">{testSmsMsg}</div>}
        </div>
      )}
        ...
        <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-6 mb-6 max-w-lg">
          ...
          <div style={{ borderTop: `1px solid ${C.border}` }} className="pt-4">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={doSaveQuickPoint} disabled={backupBusy} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="text-sm font-semibold py-2 rounded-lg disabled:opacity-50">🛡 Save Restore Point Now</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <div style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setBranchFieldsOpen(false)} style={{ color: C.textSoft }}>✕</button>
            </div>
            {branchFields.map((f) => (
              <div key={f.id} className="flex items-center justify-between py-2" style={{ borderTop: `1px solid ${C.border}` }}>
                <span style={{ color: C.ink }} className="text-sm">{f.field_name}</span>
