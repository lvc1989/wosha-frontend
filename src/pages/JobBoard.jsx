                ...
                {jobs.map((b) => {
                  ...
                  return (
                    <div key={b.id} style={{ background: "#F5F7FA" }} className="rounded-lg p-3">
                      ...
                      {canManage && (
                        <select value={b.technician_id || ""} onChange={(e) => reassignBooking(b.id, e.target.value)} style={{ borderColor: C.border }} className="w-full text-xs border rounded px-2 py-1">
                          {staffForBranch(b.location_id).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                        </select>
                      )}
                    </div>
                  );
                ...
                {extraJobs.map((j) => {
                  ...
                  return (
                    <div key={j.id} style={{ background: C.amberBg }} className="rounded-lg p-3">
                      ...
                      {canManage && (
                        <select value={j.technician_id || ""} onChange={(e) => reassignJob(j.id, e.target.value)} style={{ borderColor: C.border }} className="w-full text-xs border rounded px-2 py-1">
                          {staffForBranch(j.location_id).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                        </select>
                      )}
                    </div>
                  );
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.45)" }}>
          <form onSubmit={createJob} style={{ background: "#fff" }} className="w-full max-w-sm rounded-xl p-6">
            ...
            <select value={form.locationId} onChange={(e) => setForm({ ...form, locationId: e.target.value, technicianId: "" })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm">
              {locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
            </select>
            <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Assign to</label>
            <select required value={form.technicianId} onChange={(e) => setForm({ ...form, technicianId: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm">
              <option value="">Choose…</option>
              {staffForBranch(form.locationId).map((s) => <option key={s.id} value={s.id}>{s.name} · {s.role}</option>)}
            </select>
