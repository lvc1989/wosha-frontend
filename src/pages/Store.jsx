    ...
    <div>
      ...
      <div style={{ background: "#fff", border: `1px solid ${C.border}` }} className="rounded-xl p-5 mb-6">
        ...
        <form onSubmit={(e) => { e.preventDefault(); lookupCode(barcode); }} className="flex gap-2 mb-2">
          ...
          <button type="button" onClick={() => setScannerOpen(true)} style={{ border: `1px solid ${C.border}`, color: C.ink }} className="text-sm font-semibold px-3 rounded-lg">📷 Scan</button>
            ...
            <div key={p.id} className="flex items-center justify-between gap-2 flex-wrap px-5 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.border}` }}>
              ...
              {low && (
                <div className="flex items-center gap-2">
                  <span style={{ background: C.dangerBg, color: C.danger }} className="text-xs font-medium px-2.5 py-1 rounded-full">Low stock</span>
                  <button onClick={() => requestRestock(p)} className="text-xs font-semibold" style={{ color: C.cyan }}>Request Restock</button>
                </div>
              )}
            </div>
          );
