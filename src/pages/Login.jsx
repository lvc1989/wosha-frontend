...
function ForgotPasswordFlow({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,27,51,0.5)" }}>
      <div style={{ background: "#fff" }} className="w-full max-w-sm rounded-2xl p-7">
        <div className="flex items-center justify-between mb-4">
          <div style={{ color: C.ink }} className="text-lg font-bold">Reset Password</div>
          <button onClick={onClose} style={{ color: C.textSoft }}>✕</button>
        </div>
        ...
        {step === "otp" && (
          <>
            ...
            <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
            <PasswordStrength password={newPassword} />
            <label className="text-xs font-semibold block mb-1" style={{ color: C.textSoft }}>Confirm new password</label>
            <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-4 text-sm" />
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: C.ink }}>
      <div style={{ background: "#fff" }} className="w-full max-w-sm rounded-2xl p-7">
        ...
        {portal === "client" && (
          <div className="flex rounded-lg overflow-hidden mb-5" style={{ border: `1px solid ${C.border}` }}>
            <button onClick={() => { setMode("login"); setError(""); }} style={{ background: mode === "login" ? C.amber : "transparent", color: mode === "login" ? C.ink : C.textSoft }} className="flex-1 py-2 text-sm font-semibold">Log In</button>
            <button onClick={() => { setMode("signup"); setError(""); }} style={{ background: mode === "signup" ? C.amber : "transparent", color: mode === "signup" ? C.ink : C.textSoft }} className="flex-1 py-2 text-sm font-semibold">Create Account</button>
          </div>
        )}
        ...
        {mode === "login" || portal !== "client" ? (
          <form onSubmit={submit}>
            ...
            <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-4 text-sm" />
          ...
          <form onSubmit={submitSignup}>
            ...
            <input required type="password" value={signupForm.password} onChange={(e) => setSignupForm({ ...signupForm, password: e.target.value })} style={{ borderColor: C.border }} className="w-full border rounded-lg px-3 py-2 mb-3 text-sm" />
