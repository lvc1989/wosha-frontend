    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {summary.map((s) => {
          ...
          return (
            <div key={s.location_id} style={{ background: C.cyan }} className="rounded-2xl p-4">
              <div style={{ color: "#fff", fontFamily: "'Space Grotesk', sans-serif" }} className="text-sm font-semibold mb-3">{s.name}</div>
              <div className="flex justify-between text-xs mb-1.5 items-center">
                <span style={{ color: "rgba(255,255,255,0.75)" }}>In</span>
                <span style={{ color: "#fff", fontFamily: "'IBM Plex Mono', monospace" }}>{money(s.cash_in)}</span>
              </div>
              <div className="flex justify-between text-xs mb-1.5 items-center">
                <span style={{ color: "rgba(255,255,255,0.75)" }}>Out</span>
                <span style={{ color: "#fff", fontFamily: "'IBM Plex Mono', monospace" }}>{money(s.cash_out)}</span>
              </div>
              <div className="flex justify-between text-base font-bold mt-2 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.25)" }}>
                <span style={{ color: "#fff" }}>Net</span>
                <span style={{ color: netPositive ? "#fff" : "#FFD5D5", fontFamily: "'IBM Plex Mono', monospace" }}>{money(s.net)}</span>
              </div>
            </div>
          );
        })}
