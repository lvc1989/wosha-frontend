      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {summary.map((s) => (
          <div key={s.location_id} className="bg-white rounded-2xl p-4">
            ...
            <div className="flex justify-between text-xs mb-1.5 items-center"><span style={{ color: C.textSoft }} className="flex items-center gap-1"><ArrowDownRight size={12} color="#3B6D11" /> In</span><span style={{ color: "#3B6D11", fontFamily: "'IBM Plex Mono', monospace" }}>{money(s.cash_in)}</span></div>
            <div className="flex justify-between text-xs mb-1.5 items-center"><span style={{ color: C.textSoft }} className="flex items-center gap-1"><ArrowUpRight size={12} color={C.danger} /> Out</span><span style={{ color: C.danger, fontFamily: "'IBM Plex Mono', monospace" }}>{money(s.cash_out)}</span></div>
            <div className="flex justify-between text-sm font-bold mt-2 pt-2" style={{ borderTop: "1px solid " + C.border, color: C.ink }}><span>Net</span><span style={{ fontFamily: "'IBM Plex Mono', monospace" }}>{money(s.net)}</span></div>
          </div>
        ))}
      </div>
