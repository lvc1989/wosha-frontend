  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <button onClick={() => setChannel("all")} style={{ background: channel === "all" ? C.cyan : "#fff", color: channel === "all" ? "#fff" : C.ink, border: `1px solid ${C.border}` }} className="text-xs font-semibold px-3 py-1.5 rounded-full">All Branches</button>
        {locations.map((l) => (
          <button key={l.id} onClick={() => setChannel(l.id)} style={{ background: channel === l.id ? C.cyan : "#fff", color: channel === l.id ? "#fff" : C.ink, border: `1px solid ${C.border}` }} className="text-xs font-semibold px-3 py-1.5 rounded-full">{l.name}</button>
        ))}
      </div>
