          ...
          <>
            ...
            {showAttach && (
              <div className="mb-3">
                ...
                {attachment && <div style={{ color: "#3B6D11" }} className="text-xs mt-1">Attached: {attachment.name}</div>}
              </div>
            )}
            <div className="flex gap-2">
          ...
          {templates.map((tpl) => (
            <div key={tpl.id} style={{ background: "#F5F7FA" }} className="rounded-lg px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <button onClick={() => removeTemplate(tpl.id)} style={{ color: C.danger }}><X size={13} /></button>
              </div>
  ...
  return (
    <div>
      <div className="bg-white rounded-2xl p-5 mb-4 max-w-lg">
        ...
        {result && <div style={{ color: "#3B6D11" }} className="text-xs mt-2">Logged to {result.count} customer thread(s).</div>}
      </div>
...
export default function ClientMessages() {
  ...
  return (
    <div>
      <PageHeader title="Client messages" />
