  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button onClick={openAdd}><span className="flex items-center gap-1.5"><Plus size={16} />Add staff</span></Button>}
      />

      {staff.length === 0 ? (
        <div className="bg-white rounded-xl mb-6"><EmptyState icon={Users} title="No staff yet" body="Add your first team member to get started." action={<Button onClick={openAdd}>Add staff</Button>} /></div>
      ) : (
        <div className="flex flex-col gap-2 mb-6">
          {staff.map((s) => (
            <div key={s.id} style={{ opacity: s.active ? 1 : 0.5 }}>
              <ListRow
                icon={Users}
                tone={s.login_username ? "success" : "cyan"}
